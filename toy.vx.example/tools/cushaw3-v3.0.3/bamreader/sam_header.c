...
const char *o_rg_tags[] = { "CN", "DS", "DT", "FO", "KS", "LB", "PG", "PI",
		"PL", "PU", "SM", NULL };
...
const char **optional_tags[] = { o_hd_tags, o_sq_tags, o_rg_tags, o_pg_tags,
		NULL, NULL };
const char **required_tags[] = { r_hd_tags, r_sq_tags, r_rg_tags, r_pg_tags,
		NULL, NULL };
...
static int sam_header_line_merge_with(HeaderLine *out_hline,
		const HeaderLine *tmpl_hline) {
	list_t *tmpl_tags;
...
static HeaderLine *sam_header_line_parse(const char *headerLine) {
	...
	if (to - from != 2) {
		debug(
				"[sam_header_line_parse] expected '@XY', got [%s]\nHint: The header tags must be tab-separated.\n",
				headerLine);
		return 0;
	}
	...
	while (required_tags[itype] && required_tags[itype][itag]) {
		if (!header_line_has_tag(hline, required_tags[itype][itag])) {
			debug("The tag [%c%c] required for [%c%c] not present.\n",
					...
					required_tags[itype][itag][1], hline->type[0],
					hline->type[1]);
			return 0;
		}
		itag++;
...
void *sam_header2tbl(const void *_dict, char type[2], char key_tag[2],
		char value_tag[2]) {
...
char **sam_header2list(const void *_dict, char type[2], char key_tag[2],
		int *_n) {
	const HeaderDict *dict = (const HeaderDict*) _dict;
	const list_t *l = dict;
	int max, n;
...
void *sam_header_merge(int n, const void **_dicts) {
	...
	for (idict = 1; idict < n; idict++) {
		...
		while (tmpl_hlines) {
			...
			int inserted = 0;
			while (out_hlines) {
				status = sam_header_compare_lines(tmpl_hlines->data,
						out_hlines->data);
				if (status == 0) {
				...
				if (status == 3)
					sam_header_line_merge_with(out_hlines->data,
							tmpl_hlines->data);
			...
			if (!inserted)
				out_dict = list_append(out_dict,
						sam_header_line_clone(tmpl_hlines->data));
