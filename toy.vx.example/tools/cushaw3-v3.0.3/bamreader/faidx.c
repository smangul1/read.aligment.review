...
static inline void fai_insert_index(faidx_t *idx, const char *name, int len,
		int line_len, int line_blen, uint64_t offset) {
	khint_t k;
	int ret;
	faidx1_t t;
	if (idx->n == idx->m) {
...
faidx_t *fai_build_core(RAZF *rz) {
	...
	while (razf_read(rz, &c, 1)) {
		...
		if (c == '>') { // fasta header
			...
			len += l2;
			if (state == 1)
				line_len = l1, line_blen = l2, state = 0;
			else if (state == 0) {
...
char *fai_fetch(const faidx_t *fai, const char *str, int *len) {
	...
	razf_seek(fai->rz,
			val.offset + beg / val.line_blen * val.line_len
...
char *faidx_fetch_seq(const faidx_t *fai, char *c_name, int p_beg_i,
		int p_end_i, int *len) {
	int l;
	char c;
	khiter_t iter;
	...
	razf_seek(fai->rz,
			val.offset + p_beg_i / val.line_blen * val.line_len
					+ p_beg_i % val.line_blen, SEEK_SET);
