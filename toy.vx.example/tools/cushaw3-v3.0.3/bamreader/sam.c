...
samfile_t *samopen(const char *fn, const char *mode, const void *aux) {
	...
	if (strchr(mode, 'r')) { // read
		...
		if (strchr(mode, 'b')) { // binary
			...
			fp->x.bam =
					strcmp(fn, "-") ?
							bam_open(fn, "r") : bam_dopen(fileno(stdin), "r");
			if (fp->x.bam == 0)
				goto open_err_ret;
			...
			if (fp->header->n_targets == 0) { // no @SQ fields
				if (aux) { // check if aux is present
					...
					if (fp->header == 0)
						goto open_err_ret;
					append_header_text(fp->header, textheader->text,
							textheader->l_text);
				...
				fprintf(stderr,
						"[samopen] SAM header is present: %d sequences.\n",
						fp->header->n_targets);
		}
...
char* samget(bam_header_t* header, const bam1_t *b) {
