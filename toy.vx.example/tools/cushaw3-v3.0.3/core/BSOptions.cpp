...
void BSOptions::printUsage() {
	fprintf(stderr, "\nUsage: %s align -r bwt [options]\n", PROGRAM_NAME);
	...
	/*paired-end/mate-paired alignment*/
	fprintf(stderr, "Pairing:\n");
	fprintf(stderr,
			"\t-sensitive_pair <int> (more sensitive read pairing, default = %d)\n",
			_sensitivePairing);
	if (_estInsertSize) {
		fprintf(stderr,
...
void BSOptions::_setDefaults() {
	...
	_estInsertSize = true; /*estimate the insert size from input paired-end/mate-paired reads*/
	...
	_missProb = 0.04;
	_paired = false;
	_sensitivePairing = false;
	_maskAmbiguous = false;
	_pairingMode = PAIRED_END_READS;
	...
	_minAlginScore = 30 * _match; /*minial local alignment score*/
	_alignType = HAVE_LOCAL_SEMI_GLOBAL_ALIGN; /*using local + semiglobal alignment by default*/
...
bool BSOptions::parse(int argc, char* argv[]) {
